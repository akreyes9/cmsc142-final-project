...
class InputHandler:
    ...
    # Input Functions
    def get_input(self):
        choice = input("Input mode (1=Manual, 2=CSV): ")

        if choice == "2":
            filename = input("Enter CSV filename: ")
            tasks = self.load_from_csv(filename)

            T = self.safe_int("Enter total study time (T hours): ")
            ...
            for i in range(n):
                ...
                duration = self.safe_int("Duration (hours): ")
                deadline = self.safe_int("Deadline (hours): ")
                grade = self.safe_int("Grade: ")
